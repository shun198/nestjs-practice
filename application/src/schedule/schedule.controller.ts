...
const scheduleEmailExample = { email: 'john@example.com', sendAt: '2026-04-20T09:00:00.000Z' };
const repeatableEmailExample = { email: 'john@example.com', cronPattern: '0 0 * * *' };
...
export class ScheduleController {
  ...
  @ApiBody({ type: RepeatableEmailDto, examples: { example: { value: repeatableEmailExample } } })
  @ApiResponse({ status: HttpStatus.CREATED, description: 'ウェルカムメールの定期ジョブを登録しました' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: '指定されたメールアドレスが存在しません' })
  async registerRepeatableWelcomeEmail(@Body() dto: RepeatableEmailDto): Promise<string> {
    return this.scheduleService.registerRepeatableWelcomeEmail(dto);
  }

  @HttpCode(HttpStatus.OK)
  @Get('email/repeatable')
  @ApiResponse({ status: HttpStatus.OK, description: '登録済みの定期ジョブ一覧' })
